...
import MemberFilters from "@/app/components/MemberFilters"; 
...
export default async function MembersPage({ searchParams }: PageProps) {
  ...
  const currentSortBy = resolvedParams.sortBy || "created_at";
  const currentSortOrder = resolvedParams.sortOrder || "desc";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[12px] text-neutral-500 font-semibold tracking-wide">
            Manage your gym roster, view data logs, and recover archived entries.
          </p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-neutral-800/40 text-xs font-semibold">
              {members.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-12 text-center text-neutral-500 font-mono text-[11px]">
                    No members found matching those precise parameter structure filters.
                  </td>
                </tr>
              ) : (
                members.map((member) => (
                    ...
                    <td className="py-4 px-4 text-center">
                      <span className={`px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-wider ${
                        member.status === "active"
                          ...
                          ? "bg-neutral-500/20 text-neutral-400 border border-neutral-600/30 line-through"
        ...
        {totalPages > 1 && (
          <div className="p-4 bg-neutral-900/20 border-t border-neutral-800/60 flex items-center justify-between gap-4">
            <span className="text-[11px] text-neutral-500 font-semibold">
              ...
              <span className="ml-2 font-normal text-neutral-600">({totalCount} records matching)</span>
            </span>
